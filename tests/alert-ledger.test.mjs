...
test("watch의 알림은 원장에도 남는다 — 화면에만 있어 관제 화면이 못 보였다(2026-09-05)(card-64)", async () => {
  ...
  assert.deepEqual(result.records, [death, { ...death, resolved: true }]);
...
test("전달 실패 알림도 원장에 남는다 — 실패는 delivered:false다(card-64)", async () => {
  for (const superFails of [false, true]) {
    ...
    assert.deepEqual(result.records, [escalation, { ...death, delivered: false },
  ...
  for (const failure of ["unknown", "sent"]) {
    ...
    assert.deepEqual(result.records, [{ ...death, delivered: false },
...
test("해소는 resolved:true로 남는다 — 같은 규칙의 다른 줄이다(card-64)", async () => {
  ...
  assert.deepEqual(stdout.records, [{ ...death, recipient: null, delivered: false },
    { ...death, recipient: null, delivered: false, resolved: true }]);
  const failed = await exercise({ resolutionFails: true });
  assert.deepEqual(failed.records, [death, { ...death, delivered: false, resolved: true }]);
});
...
test("완료후보의 카드 id와 AMBER 수준도 그대로 기록한다(card-64)", async () => {
  ...
  for (const entry of completed) {
    assert.deepEqual(entry, { ...death, id: `complete-candidate:${session}:card-64|ok`, alertKind: "완료후보", level: "AMBER", taskId: "card-64" });
  }
});
