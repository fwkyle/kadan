...
test('제목 드래그는 열을 늘리고 손을 놓을 때 저장하며 정렬 클릭으로 전달하지 않는다',()=>{
 const h=harness();assert.equal(h.value('title'),320);
 assert.deepEqual(h.send('title','pointerdown'),{prevented:true,stopped:true});assert.equal(h.api.active(),true);
 h.send('title','pointermove',{clientX:600});assert.equal(h.value('title'),600);assert.equal(h.stored(),null);
 h.send('title','pointerup');assert.equal(h.api.active(),false);assert.equal(JSON.parse(h.stored()).title,600);
 assert.deepEqual(h.send('title','click'),{prevented:true,stopped:true});assert.ok(h.activities()>0);
 assert.equal(harness({saved:h.stored()}).value('title'),600);
});
...
test('취소·Escape는 드래그 전 너비를 복원하고 잘못된 포인터를 무시한다',()=>{
 ...
 for(const type of ['pointercancel','keydown']){
  ...
  assert.equal(h.api.active(),false);assert.equal(h.value('title'),320);assert.equal(h.stored(),null);assert.equal(h.body.classList.contains('dw-resizing'),false);
 }
});
test('저장소 오류나 잘못된 저장값이 화면을 막지 않고 키보드·기본 너비 복원이 동작한다',()=>{
 for(const options of [{blocked:true},{saved:'broken'},{saved:JSON.stringify({title:'600px',split:-1})}]){
  const h=harness(options);assert.equal(h.value('title'),320);
  assert.deepEqual(h.send('title','keydown',{key:'ArrowRight'}),{prevented:true,stopped:true});assert.equal(h.value('title'),336);
  h.send('title','dblclick');assert.equal(h.value('title'),320);
 }
});
