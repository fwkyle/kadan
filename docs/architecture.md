...
검토 기준: **2026-09-16**, 원장 개편과 수신자 확인·미확인 우편 감시·책임 인계의 후속 구현 코드. 아래는 코드에서 확인한 동작이며, 실제 운영 DB 전환·감시기 재시작·터미널 수신 확인을 뜻하지 않는다.
...
`readMailLedger`는 우편 사건에 더해 책임 계산용 확정 인계와 자동 결과 우편의 최종 답변 검증에 필요한 `completionMailId`가 있는 `done`을 원래 저장 순서로 반환한다. 확정 인계는 시스템 원장, `done`은 작업 원장에 각각 한 번만 저장한다. 조회 근거를 포함하기 위해 우편 원장에 사본을 저장하거나 옛 send를 고치지 않는다. 상세 조회 계약은 [원장 계약](mail-task-ledgers.md#기록과-조회-계약), 전환 순서는 [인계 안내](handover.md)를 따른다.
