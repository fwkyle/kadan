...
export function installOperationsFlow(summarize){
 ...
 const mailState=m=>[m.completion?'실행 결과 통지':'',m.read===true?'받는 사람 읽음 확인':m.read===false?'읽음 미확인':'읽음 모름',({waiting:'답변 대기',answered:'답변 완료',cancelled:'질문 취소'})[m.replyStatus]||'',m.replyFinalRejected?'전달됨 · 답변종결 반영 안됨':m.replyFinal?'최종 답장':''].filter(Boolean).join(' · ');
 ...
 function render(){
  ...
  <div class="of-situation" data-tone="${s.tone}"><strong>${escape(s.title)}</strong><p>${escape(s.description)}</p><div class="of-state-strip"><span>작업자<b>${escape(s.report)}</b></span><span>중앙<b>${escape(s.central)}</b></span><span>인수<b>${escape(s.handover)}</b></span></div></div>
 ...
 function renderEvidence(){
  ...
  if(node==='work'){
   ...
   content=`<h3>중앙 업무 마감</h3>${pill(s.central,w.status==='done'?'ok':'')}<p>${escape(explanation)}</p>${facts([['책임 감독',w.owner],['약속한 결과',w.goal]])}${w.progress?`<details><summary>기록된 진척</summary><p>${escape(w.progress)}</p></details>`:''}${w.nextAction?`<details><summary>기록된 다음 행동</summary><p>${escape(w.nextAction)}</p></details>`:''}<details><summary>완료 조건${w.result?'·최종 결과':''}</summary>${facts([['완료 조건',w.acceptance],['최종 결과',w.result]])}</details>${recordDetails(proof(`works/${w.key}/events.jsonl · 버전 ${w.revision}`,w.at,w.by))}`;
