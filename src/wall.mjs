import {ledgerView,ledgerViews} from './ledger-table.mjs';
...
function renderMailbox(mailbox, collectedAt, ledgerKnown, caption = "우편함 · 최근 20건") {
  ...
  if (!ledgerKnown || mailbox === null) {
    ...
    body = `<tbody>${mailbox
      ...
      <td>${escapeHtml(item.completion ? item.executionKey ?? item.completionTaskId ?? "-" : item.taskId ?? "-")}<div class="about">${escapeHtml(mailStatus(item))}</div>${item.title ? `<div class="about">${escapeHtml(item.title)}</div>` : ""}${item.preview ? `<div class="about">${escapeHtml(item.preview)}</div>` : ""}${item.body ? `<details class="mail-body"><summary>본문 보기</summary><pre>${escapeHtml(item.body)}</pre></details>` : ""}</td>
...
export function renderWallHtml({ tree, mailbox = [], entries = [], collectedAt, ledgerPath, ledgerLines, error, all = false, resources = null, resourceError = null, judge = false, refresh = 10, mailBoard = '', hideWatch = false, kind = '', mailRole = '', mailView = 'all', mailReply = '', mailUnread = false, ledgerDomain = 'all', home, watchPid = null, version = '', cardFiles = {} }) {
  const known = ledgerLines !== null && !entries.some(entry=>entry?.broken);
  ...
  const query = changes => {
    const values = {judge:judge?'1':'',all:all?'1':'',refresh:refresh===10?'':String(refresh),board:mailBoard,hideWatch:hideWatch?'1':'',kind,mailRole,mailView:mailView==='all'?'':mailView,mailReply,mailUnread:mailUnread?'1':'',ledgerDomain:ledgerDomain==='all'?'':ledgerDomain,...changes};
    const params = new URLSearchParams();
    for(const [key,value] of Object.entries(values)) if(value !== '') params.set(key,value);
    return '?' + params.toString();
  };
  const hidden = except => Object.entries({judge:judge?'1':'',all:all?'1':'',refresh:refresh===10?'':String(refresh),board:mailBoard,hideWatch:hideWatch?'1':'',kind,mailRole,mailView:mailView==='all'?'':mailView,mailReply,mailUnread:mailUnread?'1':'',ledgerDomain:ledgerDomain==='all'?'':ledgerDomain}).filter(([key,value])=>value!=='' && !except.includes(key)).map(([key,value])=>`<input type="hidden" name="${key}" value="${escapeHtml(value)}">`).join('');
  ...
  const ledgerTail = ledgerRows.filter(item=>!kind || item.kind===kind).slice(-50).reverse();
...
<section class="page" id="mailbox"><h2>우편함</h2><div class="panel"><form class="toolbar" method="get" action="#mailbox">${hidden(['board','hideWatch','mailRole','mailView','mailReply','mailUnread'])}<label>판 <select name="board">${options([['','모든 판'],...Array.from(new Set(allMail.map(item=>boardFromRole(item.role)).filter(Boolean))).sort().map(name=>[name,name])],mailBoard)}</select></label><label>기준 역할<input name="mailRole" value="${escapeHtml(mailRole)}" placeholder="모든 역할"></label><label>보기<select name="mailView">${options(mailViews,mailView)}</select></label><label>답변 상태<select name="mailReply">${options(replyStates,mailReply)}</select></label><label><input type="checkbox" name="mailUnread" value="1"${mailUnread?' checked':''}>읽음 미확인만</label><label><input type="checkbox" name="hideWatch" value="1"${hideWatch?' checked':''}>watch 숨기기</label><button>적용</button></form>${renderMailbox(selectedMail.slice(0,20),collectedAt,known)}</div></section><!-- end screen -->
<section class="page" id="ledger"><h2>원장</h2><p>${escapeHtml(ledgerPath ?? '모름')} · ${count(ledgerLines)}줄</p><div class="panel"><h3>최근 50줄</h3><p class="about">작업·우편·시스템은 같은 저장소 안에서 구분합니다. 과거 이력은 수정 없이 읽기용으로 분류합니다. 전체는 호환 조회이며 작업 발령의 send/dispatch를 중복 표시하지 않습니다.</p><form class="toolbar" method="get" action="#ledger">${hidden(['kind','ledgerDomain'])}<label>원장 구분<select name="ledgerDomain">${options(ledgerViews,ledgerDomain)}</select></label><label>종류 <select name="kind">${options([['','모든 종류'],...['plan','dispatch','start','send','mail-read','mail-cancel','done','stop','alert'].map(k=>[k,k])],kind)}</select></label><button>적용</button></form>${!known||ledgerError?'<p>모름 — 원장을 읽을 수 없습니다.</p>':`<div class="table-scroll"><table class="ledger-table"><thead><tr><th>시각</th><th>종류</th><th>대상</th><th>카드</th><th>누가</th><th>상세</th></tr></thead><tbody>${ledgerTail.map(item=>`<tr data-kind="${escapeHtml(item.kind)}"><td>${timeTag(item.t)}</td><td><span class="pill">${escapeHtml(item.kind)}</span></td><td>${escapeHtml(item.role ?? item.board ?? item.session ?? '-')}</td><td>${escapeHtml(item.taskId ?? '-')}</td><td>${escapeHtml(item.by ?? '모름')}</td><td class="detail">${escapeHtml(ledgerDetail(item)) || '-'}</td></tr>`).join('')||'<tr><td colspan="6">기록 없음</td></tr>'}</tbody></table></div><details><summary>원문 JSON 보기</summary><pre>${escapeHtml(ledgerTail.map(item=>JSON.stringify(item)).join('\n'))}</pre></details>`}</div></section><!-- end screen -->
...
export function createWallServer(loadSnapshot, {home,notify,cacheSec=10,now=Date.now} = {}) {
 ...
 const cached=url=>{
  ...
  if(!hit)return null;
  if(now()-hit.at>=cacheSec*1000){cache.delete(key);return null;}
  return hit;
 };
 const remember=(url,body,type,at=now())=>{
  if(!cacheSec)return;
 ...
 if (request.method === 'GET'&&!url.pathname.startsWith('/api/operations-flow')) {const hit=cached(url);if(hit){response.writeHead(200,{'content-type':hit.type,'cache-control':'no-store','x-kadan-cache':'hit'});response.end(hit.body);return;}}
    ...
    if (centerHandler && await centerHandler.handle(request,response,url)) { clear(); return; }
    const serve=()=>{
    const snapshot=sharedSnapshot&&cacheSec&&now()-sharedSnapshot.at<cacheSec*1000
      ? sharedSnapshot.value : loadSnapshot();
    if(cacheSec&&!snapshot.error&&!snapshot.centerError)sharedSnapshot={at:sharedSnapshot?.value===snapshot?sharedSnapshot.at:now(),value:snapshot};
    if(centerHandler) {
      snapshot.home=home;
    ...
    if (url.pathname === '/api/cards') {
      ...
      if(!snapshot.centerError)remember(url,body,'application/json; charset=utf-8',sharedSnapshot?.at);
    ...
    let html = renderWallHtml({
      ...
      mailRole: url.searchParams.get('mailRole') ?? '',
      mailView: url.searchParams.get('mailView') ?? 'all',
      mailReply: url.searchParams.get('mailReply') ?? '',
      mailUnread: url.searchParams.get('mailUnread') === '1',
      ledgerDomain: url.searchParams.get('ledgerDomain') ?? 'all',
      home,
