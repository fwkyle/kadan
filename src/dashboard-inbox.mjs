...
export function renderInbox(w,home,url=new URL('http://localhost'),{limit=null}={}){
 ...
 const pages=Math.max(1,Math.ceil(letters.length/listPageSize)),page=Math.min(pages,Math.max(1,Number.parseInt(url.searchParams.get('mailPage'),10)||1));
 ...
 const items=letters.slice((page-1)*listPageSize,page*listPageSize);
 ...
 }).join('')}</tbody></table></div>`}${pages>1?`<p class="bw-mail-pages">${page>1?`<a data-work-page href="${e(nav(page-1))}">이전 ${listPageSize}통</a>`:''} ${page<pages?`<a data-work-page href="${e(nav(page+1))}">다음 ${listPageSize}통</a>`:''}</p>`:''}`;
}
