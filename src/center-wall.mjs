...
export function renderCenterWall({center,centerError,collectedAt,error,resources,decisions=[],decisionError=null,entries=[],ledgerLines=0,home,ledgerState,registeredWorks,runtime}, {token='',url=new URL('http://localhost')}={}) {
 ...
 // 카드 상세만 요청하면(partial=detail) 페이지 나머지(현황·기록·우편함 등)를 그리지 않고 상세 article만 돌려준다.
 // 2026-09-24 UX 3차: 카드를 누를 때마다 2.5MB 페이지 전체를 받아 상세만 떼어 썼다. 화면 스크립트는 받은 뒤 처리가 같다.
 if(url.searchParams.get('partial')==='detail'){
  const key=url.searchParams.get('card')||'';
  const work=works?.find(w=>w.key===key||'work:'+w.key===key);
  const card=work?null:center?.cards.find(c=>c.key===key);
  return work?workDetail(work):card?detail(card):`<article id="detail" data-key=""><p role="alert">카드를 찾을 수 없습니다: ${e(key)}</p></article>`;
 }
...
${dashboardWorkspaceStyle}
 ...
 </main><div class="dw-stale" id="dw-stale" role="status" hidden><span></span><button type="button" data-refresh>지금 새로 읽기</button></div><footer><span>실행 코드 ${e(runtime?.commit||'모름')}${runtime?.dirty?' · 시작 시 미커밋 변경 있음':''} · 시작 ${e(stamp(runtime?.startedAt))}</span><span>수집 ${e(stamp(collectedAt))}</span><span id="dw-refresh-status" role="status">갱신 상태 확인 중</span><button type="button" data-refresh>새로 읽기</button></footer><script>
