...
export function workDashboardModel(works,center,entries,summaries=new Map()){
 ...
 return works.map(w=>{
  ...
  return {...w,...workHealth(w,executions),...summary,key:`work:${w.key}`,workKey:w.key,kind:'work',workType:'business',originalTitle:w.title,state:closed?w.status:w.status==='hold'?'hold':'running',stateLabel:workState[w.status],stored:w.status,
   purpose:w.goal,flowLabel:round?`${round}싸이클 · ${followup?.label||stage}`:stage,flowPhase:`실행 ${ended.length}/${executions.length}건 종료${active.length?' · 남은 실행 '+active.length+'건':''}${!closed?' · 업무는 미완료':''}`,flowTitle:w.title,
...
export function renderWorkDetail(w,{token,center,models,home,url}){
 ...
 const phaseRound=`<div class="bw-form-pair"><label>실행 단계<select name="phase">${Object.entries(workPhases).map(([k,v])=>option(k,v,'implementation')).join('')}</select></label><label>싸이클<input type="number" name="round" min="1" max="999" value="${w.round||1}" required></label></div>`;
