...
export function renderDashboardWorkspace({center,briefs,centerError,url,detail,works=null,workError=null,workDetail}) {
 ...
 const state={collection,q:params.get('q')||'',repo:params.get('repo')||'',board:params.get('board')||'',health:params.get('health')||'',rally:params.get('rally')||'',state:params.get('state')||'',layout:params.get('layout')==='split'?'split':'table',sort:params.get('sort')||'attention',dir:params.get('dir')==='asc'?'asc':'desc'};
 ...
 <script type="application/json" id="dw-data">${json({rows,selected,opened,loadedKey:work?.key||card?.key||'',state,workError,columns})}</script></section>`;
}
