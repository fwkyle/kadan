...
export function renderDashboardWorkspace({center,briefs,centerError,url,detail,works=null,workError=null,workDetail,hierarchy=null}) {
 ...
 <script type="application/json" id="dw-data">${json({rows,selected,opened,loadedKey:work?.key||card?.key||'',state,workError,columns,hierarchy})}</script></section>`;
}
